...
import "package:google_plus_v1_api/plus_v1_api_browser.dart" as plus;
...
void main() {
  ...
  plus.Plus plusclient = new plus.Plus(auth);
  ...
  void showPeople(Map peopleData) {
    plus_client.PeopleFeed people = new plus_client.PeopleFeed.fromJson(peopleData);
    ...
    if (people.items != null) {
      people.items.forEach((plus_client.Person person) {
   ...
   */
  void showProfile() {
    plusclient.people.get("me").then((plus_client.Person profile) {
  ...
  void insertItem(e) {
    String url = "${window.location.href}insertItem";
    HttpRequest.request(url, method: "POST")
    .then((HttpRequest request) {
      clientLogger.fine("insertItem from POST METHOD v = ${request.response}");
    });
  }
